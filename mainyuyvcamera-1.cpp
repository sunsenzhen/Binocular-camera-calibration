...
void initBsparam(Size imageSize)
{

    cout <<"------1------"<<endl;
    ////////////////读取矫正双目信息//////////////////////////////
    Mat cameraMatrix[2], distCoeffs[2];
    FileStorage fs("/home/sunkit/QtCreator/QtRos_ws/src/camerSM_pkg/intrinsics.yml", FileStorage::READ);
    if (fs.isOpened())
    {
        cout <<"------2------"<<endl;
    ...
    Mat ktR, ktT, ktE, ktF;
    Mat ktR1, ktR2, ktP1, ktP2, ktQ;
    Rect validRoi[2];

    // Size imageSize = Size(cameraWidth, cameraHeight);
    fs.open("/home/sunkit/QtCreator/QtRos_ws/src/camerSM_pkg/extrinsics.yml", FileStorage::READ);
    if (fs.isOpened())
    {
        fs["R"] >> ktR;
        fs["T"] >> ktT;
    ...
    stereoRectify(cameraMatrix[0], distCoeffs[0],
            ...
            imageSize, ktR, ktT, ktR1, ktR2, ktP1, ktP2, ktQ,
    ...
    initUndistortRectifyMap(cameraMatrix[0], distCoeffs[0], ktR1, ktP1, imageSize, CV_32FC1, rmap[0][0], rmap[0][1]);
    initUndistortRectifyMap(cameraMatrix[1], distCoeffs[1], ktR2, ktP2, imageSize, CV_32FC1, rmap[1][0], rmap[1][1]);
...
void remapSbimg(Mat &imagekitL,Mat &imageKitR)
{


    remap(imagekitL, imagekitL,rmap[0][0], rmap[0][1], INTER_LINEAR);
    remap(imageKitR, imageKitR, rmap[1][0], rmap[1][1], INTER_LINEAR);
...
int main()
{
  ...
  string gst = "nvcamerasrc ! video/x-raw(memory:NVMM), width=(int)640, height=(int)480, format=(string)I420, framerate=(fraction)30/1 ! nvvidconv flip-method=2 ! video/x-raw, format=(string)I420 ! videoconvert ! video/x-raw, format=(string)BGR ! appsink";
    ...
    while (true)
    {
        ...
        rgbImageL=frame(Rect(0,0,cameraWidth,cameraHeight));
        rgbImageR=frame(Rect(cameraWidth,0,cameraWidth,cameraHeight));


        if(isfixintrinsic)
        {
            remapSbimg(rgbImageL,rgbImageR);
